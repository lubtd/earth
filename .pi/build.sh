...
# docker buildx build --tag earth --file .pi/Dockerfile --platform linux/arm64 --cache-from earth:cache --cache-to earth:cache --load --progress tty .

# save the image to result-rootfs.tar
docker save --output ./.tmp/result-rootfs.tar earth
...
sudo bash -c "echo earth > ./.tmp/result-rootfs/etc/hostname"
...
# Make the image file
fallocate -l 4G "images/earth.img"

# loop-mount the image file so it becomes a disk
sudo losetup --find --show images/earth.img
